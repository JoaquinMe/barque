...
cd "$step"
for i in $(grep -v "^#" ../02_info/primers.csv | awk -F "," '{print $1}'); do
    echo -e "$step" >../10_read_dropout/"$step"_"$i"
    for j in $(ls -1 *merged_"$i"*.gz | grep "$i"); do
...
for i in $(grep -v "^#" ../02_info/primers.csv | awk -F "," '{print $1}'); do
    echo -e "$step" >../10_read_dropout/"$step"_"$i"
    for j in $(ls -1 *merged_"$i"*_unique.fasta.gz | grep "$i"); do
...
for i in $(grep -v "^#" ../02_info/primers.csv | awk -F "," '{print $1}'); do
    echo -e "$step" >../10_read_dropout/"$step"_"$i"
    grep -v "^Total" "$i"_species_table.csv | awk -F "," 'NR==1 {for (i=1; i<=NF; i++) {headers[i]=$i}} NR>1 {for (i=1; i<=NF; i++) {sums[i]+=$i}} END {for (i=6; i<=NF; i++) {print headers[i]","sums[i]}}' | sort -t "," -k1,1 -V | cut -d "," -f 2 >>../10_read_dropout/"$step"_"$i"
done
cd ..
